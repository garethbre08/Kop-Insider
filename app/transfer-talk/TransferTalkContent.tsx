...
const rumours = [
  { player: "Florian Wirtz",    club: "Bayer Leverkusen", status: "Hot",  source: "Sky Sports Germany", summary: "Liverpool leading the race ahead of Real Madrid with a £100m bid being prepared." },
  { player: "Benjamin Šeško",   club: "RB Leipzig",       status: "Warm", source: "Fabrizio Romano",    summary: "Slot personally scouted the striker twice in January. Decision expected in summer." },
  { player: "Martin Zubimendi", club: "Real Sociedad",    status: "Cold", source: "The Athletic",       summary: "Interest remains but Zubimendi prefers to stay in Spain for another season." },
];

const transferNews = [
  { id: 1, headline: "Liverpool Set to Break Transfer Record with Audacious Wirtz Bid",         excerpt: "The Reds are prepared to shatter their all-time transfer record as Slot identifies the German as his number one target for the summer window.", meta: "Andy Anfield · 1 hour ago" },
  { id: 2, headline: "Slot Wants Two Signings — A Creator and a Centre-Back Covered",           excerpt: "The Liverpool manager has given the board a clear brief heading into the summer: creativity in midfield and cover at the back.", meta: "Andy Anfield · 3 hours ago" },
  { id: 3, headline: "Núñez Future in Doubt as Serie A Giants Circle with Big Offer",           excerpt: "Juventus are preparing a €70m offer for the Uruguayan striker, though Liverpool's preference remains to keep him at Anfield.", meta: "Andy Anfield · 6 hours ago" },
  { id: 4, headline: "Academy Star Signs New Long-Term Deal Amid Interest From City",           excerpt: "Liverpool have moved quickly to tie down one of their brightest talents with a five-year contract, ending speculation over his future.", meta: "Andy Anfield · 9 hours ago" },
];
        ...
        <div className="ki-page-layout">
          ...
          <div className="ki-main">
            ...
            <section>
              ...
              <div className="ki-grid-3">
                {rumours.map((rumour, i) => (
                  <div key={i} className={`ki-card p-4 flex flex-col gap-2 animate-slide-in-left ${rumour.status === "Hot" ? "hot-glow" : ""}`} style={{ animationDelay: `${i * 100}ms` }}>
                    <div className="flex items-start justify-between gap-2">
                      <p className="ki-headline-small" style={{ marginBottom: 0 }}>{rumour.player}</p>
                      <span className={`text-xs font-semibold rounded-full px-2 py-0.5 shrink-0 ${rumourStyles[rumour.status]}`}>{rumour.status}</span>
                    </div>
            ...
            <div className="ki-opinion-strip">
              ...
              <h2 className="ki-opinion-title">The transfer window is open. Here is what Liverpool must do.</h2>
